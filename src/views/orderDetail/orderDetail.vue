  ...
  <div class="order-detail">
    ...
    <div class="content">
      ...
      <div class="info">
        ...
        <span class="item">订单状态： {{orderMsg.status === 2 ? '已完成' : orderMsg.isComplete === 1 ? '已发货' : '待发货'}}</span>
    ...
    <div class="fix-btn">
      ...
      <div :class="{isComplete: orderMsg.isComplete}" class="confirm" @click="completeOrder">{{orderMsg.isComplete ? '已完成' : '确认收货'}}</div>
    </div>
  </div>
</template>
...
import {
  getOrderBase, getOrderGoods, completeOrder, delOrder,
...
export default {
  ...
  methods: {
    completeOrder() {
      if (this.orderMsg.isComplete) {
        return;
      }
      completeOrder({ id: this.id }).then((res) => {
        if (res.status === 200) {
          this.$toast.success('订单已成功');
          this.orderMsg.isComplete = 1;
        }
      });
    },
    delOrder() {
